...
object TraverseDemo {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val currentPath = "src/main/resources/data/current/checkpoints"
        ...
        if (!lfs.exists(new Path(currentPath))) {
            df = Seq(state("src/main/resources/data/sample-data",0,"csv")).toDF()
        } else {
    ...
    def getValidations(spark: SparkSession, path:String*) : List[validation] = {
        val df = spark.read.format("csv").options(Map("inferSchema"->"true","header"->"true")).load(path:_*)
